...
* If accelerated context creation fails, retry with `{ accelerated: false }` to fall back to CPU inference through the product's approved fallback path.
...
* Requested `powerPreference` and `accelerated` are preferences, not guarantees.
