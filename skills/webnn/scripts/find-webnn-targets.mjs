...
function findWebNNMarkers(filePath) {
  let content;
  try {
  ...
  return WEBNN_MARKERS.filter((marker) => content.includes(marker));
}
...
function main() {
  ...
  for (const filePath of walkFiles(root)) {
    ...
    if (SCAN_EXTENSIONS.has(path.extname(filePath))) {
      const markers = findWebNNMarkers(filePath);
      if (markers.length > 0) {
        webnnHits.push([toPosixRelative(root, filePath), markers]);
      }
    }
  }

  scoredFiles.sort((left, right) => right[0] - left[0] || left[1].localeCompare(right[1]));
  webnnHits.sort((left, right) => left[0].localeCompare(right[0]));
  ...
  console.log("\nWebNN indicators:");
  if (webnnHits.length > 0) {
    for (const [relative, markers] of webnnHits) {
      console.log(`- ${relative}: ${markers.join(", ")}`);
    }
  } else {
    console.log("- No WebNN markers found");
  }
}
