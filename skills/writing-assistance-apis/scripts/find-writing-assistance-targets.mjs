...
function findMarkers(filePath) {
  ...
  return API_MARKERS.filter(marker => content.includes(marker));
}
...
function main() {
  ...
  for (const filePath of walkFiles(root)) {
    ...
    if (SCAN_EXTENSIONS.has(path.extname(filePath))) {
      ...
      if (markers.length > 0) {
        apiHits.push([toPosixRelative(root, filePath), markers]);
      }
    }
  }

  scoredFiles.sort((left, right) => right[0] - left[0] || left[1].localeCompare(right[1]));
  apiHits.sort((left, right) => left[0].localeCompare(right[0]));
  ...
  console.log("\nWriting Assistance API indicators:");
  if (apiHits.length > 0) {
    for (const [relative, markers] of apiHits) {
      console.log(`- ${relative}: ${markers.join(", ")}`);
    }
  } else {
    console.log("- No Writing Assistance API markers found");
  }
}
