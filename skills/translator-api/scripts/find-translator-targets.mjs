...
function findTranslatorMarkers(filePath) {
  let content;
  try {
  ...
  return TRANSLATOR_MARKERS.filter(marker => content.includes(marker));
}
...
function main() {
  ...
  for (const filePath of walkFiles(root)) {
    ...
    if (SCAN_EXTENSIONS.has(path.extname(filePath))) {
      const markers = findTranslatorMarkers(filePath);
      if (markers.length > 0) {
        translatorHits.push([toPosixRelative(root, filePath), markers]);
      }
    }
  }

  scoredFiles.sort((left, right) => right[0] - left[0] || left[1].localeCompare(right[1]));
  translatorHits.sort((left, right) => left[0].localeCompare(right[0]));
  ...
  console.log("\nTranslator API indicators:");
  if (translatorHits.length > 0) {
    for (const [relative, markers] of translatorHits) {
      console.log(`- ${relative}: ${markers.join(", ")}`);
    }
  } else {
    console.log("- No Translator API markers found");
  }
}
