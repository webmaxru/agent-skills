...
const LANGUAGE_DETECTOR_MARKERS = [
  "LanguageDetector",
...
function findLanguageDetectorMarkers(filePath) {
  let content;
  try {
  ...
  return LANGUAGE_DETECTOR_MARKERS.filter(marker => content.includes(marker));
}
...
function main() {
  ...
  for (const filePath of walkFiles(root)) {
    ...
    if (SCAN_EXTENSIONS.has(path.extname(filePath))) {
      const markers = findLanguageDetectorMarkers(filePath);
      if (markers.length > 0) {
        languageDetectorHits.push([toPosixRelative(root, filePath), markers]);
      }
    }
  }

  scoredFiles.sort((left, right) => right[0] - left[0] || left[1].localeCompare(right[1]));
  languageDetectorHits.sort((left, right) => left[0].localeCompare(right[0]));
  ...
  console.log("\nLanguage Detector API indicators:");
  if (languageDetectorHits.length > 0) {
    for (const [relative, markers] of languageDetectorHits) {
      console.log(`- ${relative}: ${markers.join(", ")}`);
    }
  } else {
    console.log("- No Language Detector API markers found");
  }
}
