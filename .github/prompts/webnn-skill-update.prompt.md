---
name: "WebNN: Update Skill"
description: "Refresh the skills/webnn skill from user-supplied textual updates, attached documents, and URLs about the current WebNN surface. Use when browser guidance, compatibility notes, or implementation details may have changed and the skill, references, assets, or examples need to be reconciled."
...
Update the `skills/webnn` skill in this workspace using the latest available inputs and any textual updates the user supplies.
...
3. Built-in prompt URLs fill gaps not covered by higher-priority sources, using `https://www.w3.org/TR/webnn/` as the primary URL source and `https://webnn.io/en/api-reference/reference` as the secondary URL source.
...
- Use the built-in prompt URLs to fill gaps in API semantics, browser support notes, device behavior, and runtime guidance that are not covered or are less detailed in the higher-priority sources.
- Prefer the W3C WebNN specification over the secondary reference URL when both cover the same surface and no higher-priority source overrides them.
...
- Read the current skill files under [skills/webnn](../../skills/webnn/), including `SKILL.md`, `references/`, `assets/`, and `scripts/`.
...
2. Extract only material changes that affect the skill, such as:
   - `navigator.ml` exposure requirements, secure-context restrictions, and execution-context support
   - `ML.createContext()` or equivalent context creation behavior, including `deviceType` and `powerPreference`
   - `MLGraphBuilder`, operand typing, graph build constraints, tensor write-dispatch-read flows, and readback behavior
   - supported `Window` and `DedicatedWorker` usage, transfer limitations, and lifecycle concerns
   - backend variability, device fallback behavior, graph partitioning, or operator-support limits
   - preview browser requirements, flags, platform support, and implementation-specific caveats
   - examples that materially affect the recommended runtime shape, fallback path, or validation workflow
   - removed, renamed, or obsolete WebNN surfaces that should be corrected in the skill or references
3. Compare the extracted changes against the current files in `skills/webnn`.
...
6. Preserve the skill's routing quality:
   ...
   - local inference and runtime integration guidance only
   - no model training guidance
   - no server-side ML inference guidance
...
- Update `assets/webnn-runtime.template.ts` only if the canonical usage pattern has materially changed.
- Update `scripts/find-webnn-targets.mjs` only if the existing detection logic is actually wrong for the skill workflow.
